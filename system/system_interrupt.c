...
/*******************************************************************************
 * Function     : void __attribute__((__interrupt__)) _ADCAN4Interrupt( void ))
 *
...
*
* Description  : Service Routine (ISR) for the UART Rx interrupt.
*                This routine calls the UART's Rx interrupt routine to
*                service the interrupt.
*
* Precondition : The UART peripheral must have been initialized.
*
* Input        : None
*
* Output       : None
...
void __attribute__ ( ( __interrupt__ , auto_psv ) ) _U1RXInterrupt ( void )
{
    if (U1STAbits.OERR == 1) {
        U1STAbits.OERR = 0;
        PROCESS_PostInterruptEvent(INT_UART_OERROR);
    } else {
        uint8_t tempRx;
        while (U1STAHbits.URXBE == 0) {
            if ((U1STAbits.FERR == 1) || (U1STAbits.PERR == 1)) {
                    PROCESS_PostInterruptEvent(INT_UART_ERROR);
                    tempRx = U1RXREG;
            } else {
                tempRx = U1RXREG;
                if (!appData.uartRxBuffFull) {
                    appBuffers.uartReceive[appData.uartRxTailIndex] = tempRx;
                    appData.uartRxTailIndex++;
                    appData.uartRxTailIndex &= (SERIAL_BUFFER_MAX_LENGTH-1);
                    appData.uartRxBuffEmpty = 0;
                    if (appData.uartRxTailIndex == appData.uartRxHeadIndex) appData.uartRxBuffFull = 1;
                }
            }
        }
        PROCESS_PostInterruptEvent(INT_UART_RX_READY);
    }

    IFS0bits.U1RXIF = 0; /* Clear UART01 Rx interrupt */
}

 /*******************************************************************************
 * Function     : void __attribute__((__interrupt__)) _U1TXInterrupt( void ))
 *
 * Description  : Service Routine (ISR) for the UART Tx interrupt.
 *                This routine calls the UART's Tx interrupt routine to
 *                service the interrupt.
 *
 * Precondition : The UART peripheral must have been initialized.
 *
...
void __attribute__ ( ( __interrupt__ , auto_psv ) ) _U1TXInterrupt ( void )
{
    PROCESS_PostInterruptEvent(INT_UART_TX_READY);

    IFS0bits.U1TXIF = 0; /* Clear UART01 Tx interrupt */
}
